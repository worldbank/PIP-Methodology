...
Data at the bottom of consumption and income distributions are typically prone to measurement errors [@ravallion2016]. Zero (or very low) consumption is not plausible, given that a minimum consumption of at least food is required for human survival. For income surveys, very low, zero, and even negative incomes are more plausible as individuals can finance consumption by drawing down savings.

All poverty and inequality indicators in PIP are calculated using income and consumption distribution that drop negative incomes and replace all other observations below $\$0.25$ per person per day are replaced with $\$0.25$ per person per day (Yonzan et al. (forthcoming)).
