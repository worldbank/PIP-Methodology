...
Some economies have no household survey data that can be used to monitor poverty (or they lack the national accounts data for a particular reference year). For the regional and global aggregations, these economies are assigned the population-weighted average for the region based on the economies with data available. Population data are taken from the World Bank’s [World Development Indicators](http://data.worldbank.org/products/wdi).Regions are defined using the PovcalNet classification, which differs from the regional classifications typically used by the World Bank (see "Regions and universe of countries").
