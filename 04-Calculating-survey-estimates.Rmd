...
Armed with welfare distributions expressed in 2011 PPPs and an international poverty line, poverty and inequality can be calculated and compared across countries and over time. The poverty and inequality measures used in the Poverty and Equity Platform for micro data are mainly based on formulae in @haughton2009, while the poverty and inequality measures estimated from grouped data are mainly based on formulae in @datt1998. In the formulas below we abstract from household weights.
...
An individual is considered poor if she/he lives in a household that is deprived in at least 1/3 of the weighted indicators.\
\
...
In the same vein of the Watts Index, the MLD suffers from being undefined when there are zero values in the distribution. Currently the Poverty and Equity Platform replaces zeros with ones when in order to use the same sample as with the other inequality measures.

**Polarization**: The polarization index ($P$), also known as the Wolfson polarization index, measures the extent to which the distribution of welfare is "spread out" and bi-modal. It is given as ([source code]{style="color:red"}):
...
where $\mu_{b40,t_2}$ and $\mu_{b40,t_1}$ is the mean welfare of the bottom 40% at time $t_2$ and \$t_1\$. Shared prosperity is anonymized, meaning that it does not track the same individuals in both time periods.
