...
where $N$ is the total population size, $μ$ is the mean welfare per person, and $y_i$ is welfare of individual $i$. The MLD has a minimum value of 0 (perfect equality) and has no upper bound. This formula applies for both micro and grouped data ([source code](https://github.com/PIP-Technical-Team/wbpip/blob/master/R/md_compute_mld.R)).
...
**Prosperity gap**
The prosperity gap (PG) is the average factor by which incomes fall short of a prosperity standard of \$25 per person per day (expressed in 2017 PPP dollars) [@kraay2023]. It is given by:

$$ PG  = \frac{1}{N} \sum_{i=1}^{N} \frac{25}{y_i}. $$
For a person whose welfare is $\$2.50$ per day, this ratio is $10$, meaning that their welfare falls short of the prosperity standard by a factor of $10$. This measure unambiguously gives greater weight to poorer people. The poorest person with daily welfare equal to $\$2.50$ gets twice the weight of a person with twice the welfare (at $\$5.00$), and ten times the weight of a person with ten times higher welfare (at $\$25$).
